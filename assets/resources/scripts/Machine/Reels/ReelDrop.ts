...
export class ReelDrop extends Component {
  ...
  spinAgain() {
    ...
    for (let i = this.reelAnchor.children.length - 1; i >= 0; i--) {
      ...
      let position =
        (this.node.getComponent(UITransform).height + element.getComponent(UITransform).height) * 0.5 * direction +
        element.getComponent(UITransform).height * (this.reelAnchor.children.length - 1 - i);
    ...
    tween(element)
      .by(
        10,
        {
          position: new Vec3(
            0,
            this.maskedHeight * 0.5 * dirModifier * element.getComponent(UITransform).height * this.count,
            0
          ),
        },
        { easing: "bounceOut" }
